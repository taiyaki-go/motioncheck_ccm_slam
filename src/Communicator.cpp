    ...
    while(true)
    {
...

        {
            ...
            if(!mlBufKFin.empty())
            {
                ...
                ofs.open("/home/muto/ccmslam_ws/src/ccm_slam/cslam/output/output_commtime.txt");
                ofs << msec << "msec\n" << endl;
                ofs.close();
            }
...
void Communicator::PublishMapClient()
{
    ...
    //Keyframes
    {
        ...
        while(!mspBufferKfOut.empty() && (ItCount < mKfItBoundPub)) //Do not call checkBuffer()-method -- it also need the BufferOutMutex
        {
            ...
            // for debug
            //cout << "degree (published) : " << pCurKf->xy_deg << ", " << pCurKf->yz_deg << ", " << pCurKf->zx_deg << endl;
