...
//    g2o::Sim3 g2oScur(Converter::toMatrix3d(Rcur),Converter::toVector3d(tcur),1.0);
    ...
    for(KeyFrameAndPose::iterator mit=CorrectedSim3.begin(), mend=CorrectedSim3.end(); mit!=mend; mit++)
    {
        ...
        /*
        cout << "rotation matrix (SE3) : " << endl;
        cout << eigR << endl;
        cout << "translation matrix (SE3) : " << endl;
        cout << eigt << endl;
        cout << "scale (SE3) : " << s << endl;
        cout << endl;
        */
        ////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        // add this code (2023/12/17) //////////////////////////////////////////////////////////////////////////
        //pKFi->Tcw_current = pKFi->GetPose();
        //cout << "before correct loop : " << endl;
        //cout << pKFi->GetPose() << endl;
        ////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        // add this code (2023/12/17) //////////////////////////////////////////////////////////////////////////
        //cout << "after correct loop : " << endl;
        //cout << pKFi->GetPose() << endl;
        //cout << endl;
        //pKFi->Tcw_new = pKFi->GetPose();

        //pKFi->isLoopClosing = true;
        ////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    if (check_motion_violate_KF_switch == 1) {
        ...
        {
            std::stringstream ss;
            ss << params::stats::msOutputDir << "KF_GBA_" << it << "_LC" << loop_finder_counter << "_before.csv";
            mpMap->WriteStateToCsv(ss.str(),it);
        }
        /*
        std::map<idpair,kfptr> mmpKeyFrames = mpMap->GetMmpKeyFrames();
        std::cout << "KFs in Map (before Motion Check) : " << mmpKeyFrames.size() << endl;
        mpMap->CheckMotionViolateKF_before(0);
        mmpKeyFrames = mpMap->GetMmpKeyFrames();
        ...
        */
        ...
        {
            std::stringstream ss;
            ss << params::stats::msOutputDir << "KF_GBA_" << it << "_LC" << loop_finder_counter <<"_beforeBA.csv";
            mpMap->WriteStateToCsv(ss.str(),it);
        }
        */
    }
    ...
    if (mpMap->request_GBA == true) {
        std::cout << "LoopFinder: request Map GBA - launch thread" << std::endl;
        mpMap->unsetNoStartGBA();
        mpMap->mbStopGBA = false;
        mpMap->RequestBA_fromCorrectKF(0);
        mpMap->mbStopGBA = true;
        mpMap->request_GBA = false;
        std::cout << "LoopFinder: reGBA finished - continue" << std::endl;
    }
        ...
        if (check_motion_violate_KF_switch == 1) {
            ...
            {
                std::stringstream ss;
                ss << params::stats::msOutputDir << "KF_GBA_" << it << "_LC" << loop_finder_counter << "_after.csv";
                mpMap->WriteStateToCsv(ss.str(),it);
            }
            /*
            std::map<idpair,kfptr> mmpKeyFrames = mpMap->GetMmpKeyFrames();
            std::cout << "KFs in Map (before Motion Check) : " << mmpKeyFrames.size() << endl;
            mpMap->CheckMotionViolateKF_after(0);
            mmpKeyFrames = mpMap->GetMmpKeyFrames();
            std::cout << "KFs in Map (after Motion Check) : " << mmpKeyFrames.size() << endl;
           ...
           /*
            ...
            {
                std::stringstream ss;
                ss << params::stats::msOutputDir << "KF_GBA_" << it << "_LC" << loop_finder_counter <<"_afterMC.csv";
                mpMap->WriteStateToCsv(ss.str(),it);
            }
            */
        }
...
void LoopFinder::RunGBA(idpair nLoopKF, set<idpair> sChangedKFs)
{
    ...
    {
        std::stringstream ss;
        ss << params::stats::msOutputDir << "KF_GBA_" << it << "_LC" << loop_closing_counter << "_before.csv";
        mpMap->WriteStateToCsv(ss.str(),it);
    }
    */
    ...
    */
    ...
    //mpMap->SetTcwCurrent(0, mpMap->GetMmpKeyFrames());
...
//        cout << "Changed KF poses: " << sChangedKFs.size() << endl;
    ...
    {
        std::stringstream ss;
        ss << params::stats::msOutputDir << "KF_GBA_" << it << "_LC" << loop_finder_counter <<"_afterBA.csv";
        mpMap->WriteStateToCsv(ss.str(),it);
    }
    */
    // add this code (2023/12/11) ////////////////////////////////////////////////////////
    //mpMap->CheckMotionViolateKF_after(0);
    //mpMap->CorrectKF(0, false);

    /*
    mpMap->SetTcwNew(0, mpMap->GetMmpKeyFrames());
    std::vector<kfptr> foundKFs;
    foundKFs.reserve((mpMap->GetMmpErasedKeyFrames()).size());
    // Get all frames from the required client
    for (std::map<idpair,kfptr>::const_iterator itr =(mpMap->GetMmpErasedKeyFrames()).begin();
        itr != (mpMap->GetMmpErasedKeyFrames()).end(); ++itr) {
    ...
    */
    ...
    //mpMap->CorrectKF_V2(0, mpMap->GetMmpKeyFrames(), false, false);
    //mpMap->CorrectKF_V2(0, mpMap->GetMmpKeyFrames(), false, true);
    ...
    mpMap->SetTcwNew(0, mpMap->GetMmpKeyFrames());
    mpMap->CorrectKF_V3(0, mpMap->GetMmpKeyFrames(), false, false);
    mpMap->CorrectKF_V3(0, mpMap->GetMmpKeyFrames(), false, true);

    end = std::chrono::system_clock::now();
    auto time = end - start;
    auto msec = std::chrono::duration_cast<std::chrono::milliseconds>(time).count();
    ofs.open("/home/muto/ccmslam_ws/src/ccm_slam/cslam/output/output_MC_LCtime.txt");
    ofs << msec << "msec\n" << endl;
    ofs.close();
    //////////////////////////////////////////////////////////////////////////////////////
    ...
    if(params::stats::mbWriteKFsToFile)
    {
        ...
        {
            std::stringstream ss;
            ss << params::stats::msOutputDir << "KF_GBA_" << it << "_LC" << loop_closing_counter << "_after.csv";
            mpMap->WriteStateToCsv(ss.str(),it);
        }
        */
    }
