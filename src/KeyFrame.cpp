...
KeyFrame::KeyFrame(ccmslam_msgs::KF* pMsg, vocptr pVoc, mapptr pMap, dbptr pKFDB, commptr pComm, eSystemState SysState, size_t UniqueId, g2o::Sim3 mg2oS_wcurmap_wclientmap, float tracking_x, float tracking_y, float tracking_z) // modified (2023/11/16)
    : mFrameId(defpair),mVisId(-1),
      mbFirstConnection(true),mbNotErase(false),
      mbToBeErased(false),
      mpORBvocabulary(pVoc),mpMap(pMap), mpKeyFrameDB(pKFDB),
...
// add this code (2023/12/8) //////////////////////////////////////////////////////////////////////////

cv::Mat KeyFrame::GetPose_GBABef()
{
    unique_lock<mutex> lock(mMutexPose);
    return mTcwBefGBA.clone();
}

cv::Mat KeyFrame::GetPoseInverse_GBABef()
{
    unique_lock<mutex> lock(mMutexPose);
    cv::Mat mTwcBefGBA = cv::Mat::eye(4,4,mTcwBefGBA.type());
    return mTwcBefGBA.clone();
}
...
cv::Mat KeyFrame::GetTranslation_GBABef()
{
    unique_lock<mutex> lock(mMutexPose);
    return mTcwBefGBA.rowRange(0,3).col(3).clone();
}

cv::Mat KeyFrame::GetTranslation_GBAAft()
{
    unique_lock<mutex> lock(mMutexPose);
    return mTcwGBA.rowRange(0,3).col(3).clone();
}
...
void KeyFrame::ConvertToMessage(ccmslam_msgs::Map &msgMap, g2o::Sim3 mg2oS_wcurmap_wclientmap, kfptr pRefKf, bool bForceUpdateMsg)
{
    ...
    if((mbSendFull || mSysState == eSystemState::SERVER) && !bForceUpdateMsg)
    {
        ...
        else
        {
            ...
            // add this code (2023/11/11) /////////////////////////////////////////////////////////////////////////////////////////////
            //Msg.deg = this->deg;

            //Msg.tracking_x = this->tracking_x;
            //Msg.tracking_y = this->tracking_y;
            //Msg.tracking_z = this->tracking_z;

            // for debug
            //cout << "degree (In Msg) : " << Msg.xy_deg << ", " << Msg.yz_deg << ", " << Msg.zx_deg << endl;
...
void KeyFrame::WriteMembersFromMessage(ccmslam_msgs::KF *pMsg, g2o::Sim3 mg2oS_wcurmap_wclientmap)
{
    ...
    //tracking_x=pMsg->tracking_x;
    //tracking_y=pMsg->tracking_y;
    //tracking_z=pMsg->tracking_z;

    // for debug
    //cout << "degree (received) : " << xy_deg << ", " << yz_deg << ", " << zx_deg << endl;
    ...
    else if(mSysState == eSystemState::SERVER)
    {
        ...
        if(this->mId.first != 0)
        {
            ...
            // add this code (2023/12/15) ///////////////////////////////////////////////////////////
            const cv::Mat T_wc_current = cv::Mat::eye(4,4,Tcw.type());
            cv::Mat Rcw = Tcw.rowRange(0,3).colRange(0,3);
            cv::Mat tcw = Tcw.rowRange(0,3).col(3);
            cv::Mat Rwc = Rcw.t();
            Ow = -Rwc*tcw;
            ...
            const Eigen::Matrix4d Tws_current = eT_wc_current * mT_SC.inverse();

            tracking_x = Tws_current(0,3);
            tracking_y = Tws_current(1,3);
            tracking_z = Tws_current(2,3);
            //cout << "tracking_x : " << tracking_x << endl;
            //cout << "tracking_y : " << tracking_y << endl;
            //cout << "tracking_z : " << tracking_z << endl;
            //cout << "Tcw : " << Tcw;
            
            if (set_original == false) {
                Tcw_original = Tcw;
                Tcw_current = Tcw;
                set_original = true;
            }
